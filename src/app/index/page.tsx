...
export default function Index() {
  ...
  return (
    <div className="relative top-96 absolute">
        <div className="">
            <Swiper
                ...
                loop // 무한 루프
                className="h-96 w-[80vw] rounded-lg" // 높이 설정
            >
                {/* 5개의 카드 슬라이드 추가 */}
                {contents.map((item) => (
                <SwiperSlide key={item}>
                    <div className="flex justify-center items-center bg-black opacity-80 h-full text-white">
                    <h3 className="text-2xl p-10">{item}</h3>
                    </div>
                </SwiperSlide>
                ))}
            </Swiper>
